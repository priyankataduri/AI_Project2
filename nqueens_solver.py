...
def input_queens():
    """
    Prompt user to input the number of queens, ensuring a valid value (>= 4).
    
    Returns:
        int: Number of queens for the N-Queens problem.
    """
    while True:
...
def generate_random_state(n):
    """
    Generate a random initial state where each queen is placed in a random row.
    
    Args:
        n (int): Number of queens (and board size).
    
    Returns:
        list: A list representing queen positions in each column.
    """
...
def calculate_heuristic(state):
    ...
    Args:
        state (list): A list representing queen positions in each column.
    
    Returns:
        int: Heuristic value (number of attacking pairs).
    """
    n = len(state)
    h = 0
...
def print_board(state):
    """
    Print the board representation of the given state.
...
def get_successors(state):
    ...
    Returns:
        list: List of possible successor states.
    """
    n = len(state)
    successors = []
    for col in range(n):
...
def basic_hill_climbing(n, max_sideways=0):
    """
    Perform Hill Climbing algorithm with optional sideways moves.
    
    Args:
        n (int): Number of queens.
        max_sideways (int): Maximum allowed sideways moves.
    
    Returns:
        tuple: (Success status, steps taken, final state, path)
    """
...
def random_restart_hill_climbing(n, use_sideways=False):
    """
    Perform Random-Restart Hill Climbing with optional sideways moves.
    
    Args:
        n (int): Number of queens.
        use_sideways (bool): Allow sideways moves.
    
    Returns:
        tuple: (Success status, total steps, final state, path)
    """
...
def main():
    """
    Main function to execute the N-Queens Hill Climbing solver.
    """
